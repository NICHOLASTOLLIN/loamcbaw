...
router.post(
  ...
  [
    ...
    body('iconUrl').optional().trim().isURL({ require_protocol: true }).withMessage('Icon URL must be a valid URL'),
  ],
  async (req, res) => {
    ...
    const { title, url, icon, iconUrl } = req.body;
...
router.put(
  ...
  async (req, res) => {
    ...
    const allowedFields = ['title', 'url', 'icon', 'iconUrl', 'isActive'];
    const updates = {};
